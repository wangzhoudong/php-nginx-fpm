...
if   [   $FPM_CONFIG   ] &&  [ ! -z "$FPM_CONFIG" ]; then
#  echo '执行了系统环境变量的的配置文件切换'
	cp /usr/local/etc/php-fpm.d.defualt/${FPM_CONFIG}.conf /usr/local/etc/php-fpm.d/www.conf
fi

if   [   $1   ] &&  [ ! -z "$1" ]; then
#  echo '执行了参数传递的配置文件切换'
	cp /usr/local/etc/php-fpm.d.defualt/${1}.conf /usr/local/etc/php-fpm.d/www.conf
fi
